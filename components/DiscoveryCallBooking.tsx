...
import { FiCalendar, FiClock, FiUser, FiMail, FiPhone, FiChevronLeft, FiChevronRight } from 'react-icons/fi';
...
const DiscoveryCallBooking = () => {
  ...
  // Generate time slots from 8 AM to 12 PM in 30-minute increments
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    alert('Your discovery call has been booked! We will send you a confirmation email shortly.');
  };
        ...
        <div className="flex flex-col lg:flex-row">
          ...
          <div className="lg:w-3/12 lg:pr-8 mb-10 lg:mb-0">
            <div className="bg-indigo-50 border border-indigo-100 rounded-xl p-6 sticky top-8">
              <h3 className="text-xl font-bold text-indigo-700 mb-4">Limited-Time Offer</h3>
              <p className="text-gray-700 mb-6">
                Book a call now for your 100% FREE no-strings-attached recruitment strategy session and as a thank you we'll send you:
              </p>
              ...
              <ul className="space-y-4 mb-8">
                <li className="flex items-start">
                  ...
                  <span className="text-gray-700">5 Lessons on Building A Winning Team</span>
                </li>
                <li className="flex items-start">
                  <svg className="w-5 h-5 text-indigo-600 mt-0.5 mr-2 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 13l4 4L19 7" />
                ...
                <li className="flex items-start">
                  ...
                  <span className="text-gray-700">100+ Templates for Hiring, Onboarding and Training</span>
                </li>
                <li className="flex items-start">
                  <svg className="w-5 h-5 text-indigo-600 mt-0.5 mr-2 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 13l4 4L19 7" />
          ...
          <div className="lg:w-9/12 lg:pl-8">
            <div className="bg-gray-50 rounded-xl p-8 border border-gray-200">
              <h2 className="text-2xl font-bold text-gray-900 mb-2">Book Your Discovery Call</h2>
                      ...
                      <button
                        ...
                        className={`h-10 rounded-lg flex items-center justify-center text-sm transition-colors
                          ${isSelected ? 'bg-indigo-600 text-white' : 
                            isToday ? 'border border-indigo-300 bg-indigo-50 text-indigo-700' : 
                            isAvailable ? 'hover:bg-gray-200 text-gray-700' : 'text-gray-400 cursor-not-allowed'}
                        `}
                      >
                        {date.getDate()}
                      </button>
              ...
              {selectedDate && (
                <div className="mb-6 p-4 bg-indigo-50 rounded-lg">
                  <h3 className="font-medium text-indigo-800 mb-1">Selected Date</h3>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                    {availableSlots[selectedDate.toISOString().split('T')[0]]?.map(time => (
                      <button
                        ...
                        className={`py-2 px-3 rounded-lg border text-center transition-all ${selectedTime === time ? 'border-indigo-600 bg-indigo-50 text-indigo-700' : 'border-gray-300 hover:border-indigo-300'}`}
                      >
                        {time}
                      </button>
                    ))}
                  ...
                  <div className="space-y-6">
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-3">
                        What role are you looking to hire first? (Select all that apply)
                      </label>
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-3">
                        I'm a Digital Marketing/SAAS Agency Owner
                      </label>
                      <div className="flex space-x-4">
                        <label className="inline-flex items-center">
                          <input
                            ...
                            checked={formData.isAgencyOwner === 'yes'}
                            onChange={() => setFormData({...formData, isAgencyOwner: 'yes'})}
                          />
                        ...
                        <label className="inline-flex items-center">
                          <input
                            ...
                            name="agencyOwner"
                            className="h-4 w-4 text-indigo-600 focus:ring-indigo-500"
                            checked={formData.isAgencyOwner === 'no'}
                            onChange={() => setFormData({...formData, isAgencyOwner: 'no'})}
                          />
                    ...
                    <div>
                      ...
                      <textarea
                        id="note"
                        rows={3}
                        className="block w-full border border-gray-300 rounded-lg focus:ring-indigo-500 focus:border-indigo-500"
                    ...
                    <div className="pt-2">
                      <button
                        type="submit"
                        className="w-full px-6 py-3 bg-gradient-to-r from-blue-600 to-indigo-600 text-white font-medium rounded-lg hover:from-blue-700 hover:to-indigo-700 transition-all shadow-lg hover:shadow-xl"
                      >
